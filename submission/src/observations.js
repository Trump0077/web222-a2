...
function observationSummary(data) {
  ...
  for (let i = 0; i < data.results.length; i++) {
    let e = results[i];
    console.log(`#${e.id} - ${e.species_guess} [${e.observed_on_details.date}]`);
  }
}
...
function observationsByPrivacy(data, privacy) {
  ...
  if (privacy !== null && typeof privacy === 'string') {
    data.results.forEach((e) => {
      if (e.privacy === privacy || e.privacy === privacy.toLowerCase()) {
        newArr.push(e);
      }
    });
...
function transformObservation(original) {
  ...
  for (let [key, value] of Object.entries(original)) {
    ...
    if (key === 'species_guess') {
      newObject.speciesGuess = value;
    }
    if (key === 'quality_grade') {
      if (value === 'research') {
        newObject.isResearchQuality = true;
      } else {
        newObject.isResearchQuality = false;
      }
    }
    if (key === 'location') {
      let arrCoords = value.split(',');
      let convert = [];
      convert[0] = parseFloat(arrCoords[1]);
      convert[1] = parseFloat(arrCoords[0]);
      newObject.geoCoords = convert;
    }
    if (key === 'photos') {
      value.forEach((e) => {
        for (const [key, url] of Object.entries(e)) {
          if (key === 'url') {
            arrPhotoUrls.push(url);
          }
        }
      });
      newObject.photosCount = arrPhotoUrls.length;
    }
    if (key === 'user') {
      newObject.user = '@' + value.login_exact;
    }
  }
  newObject.photoUrls = arrPhotoUrls;
  return newObject;
}
...
function transformObservations(data) {
  let newArr = [];
  data.results.forEach((e) => {
    newArr.push(transformObservation(e));
  });
  return newArr;
}
...
function transformObservations2(data) {
  let newArr = data.results.map(function (result) {
...
function getObservationsById(data, ...ids) {
  ...
  ids.forEach((e) => {
    ...
    object = data.results.find(check);
    if (object !== undefined) {
      newArr.push(object);
    }
  });
...
function getObservationsByPositionalAccuracy(data, options = {}) {
  if (Object.keys(options).length !== 0) {
    return data.results.filter(function (e) {
      return options.eq
        ? e.positional_accuracy === options.eq
        : options.gte && options.lte
        ? e.positional_accuracy >= options.gte && e.positional_accuracy <= options.lte
        : options.gte
        ? e.positional_accuracy >= options.gte
        : options.lte
        ? e.positional_accuracy <= options.lte
        : false;
    });
  }
  return data.results;
...
function getTaxonPhotos(data) {
  ...
  let obId = data.results[0].id;
  data.results.forEach((e) => {
    if (e.taxon) {
      if (e.taxon.default_photo) {
        let photoId = e.taxon.default_photo.id;
        convert.original = `https://static.inaturalist.org/photos/${photoId}/original.jpg?${obId}`;
        convert.square = `https://static.inaturalist.org/photos/${photoId}/square.jpg?${obId}`;
        convert.small = `https://static.inaturalist.org/photos/${photoId}/small.jpg?${obId}`;
        convert.medium = `https://static.inaturalist.org/photos/${photoId}/medium.jpg?${obId}`;
        convert.large = `https://static.inaturalist.org/photos/${photoId}/large.jpg?${obId}`;
        newArr.push(convert);
      }
    }
  });
...
function getUserStats(data) {
  ...
  let userSpecs = 0;
  data.results.forEach((e) => {
    userObs += e.user.observations_count;
    userJournals += e.user.journal_posts_count;
    userSpecs += e.user.species_count;
    userCount++;
  });
  newObject.count = userCount;
  newObject.totals.observations = userObs;
  newObject.totals.journals = userJournals;
  newObject.totals.species = userSpecs;
  newObject.averages.observations = userObs / userCount;
  newObject.averages.journals = userJournals / userCount;
  newObject.averages.species = userSpecs / userCount;
  return newObject;
}
...
function extractTimeZones(data) {
  let newArr = [];
  data.results.forEach((e) => {
    if (!newArr.includes(e.created_time_zone)) {
      newArr.push(e.created_time_zone);
    }
  });
  return newArr;
}
...
function extractTimeZones2(data) {
  let newSet = new Set();
  data.results.forEach((e) => {
    newSet.add(e.created_time_zone).add(e.observed_time_zone);
  });
